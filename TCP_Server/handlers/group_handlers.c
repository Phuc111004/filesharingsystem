...
void handle_join_req(MYSQL *db, int current_user_id, const char *arg1, char *response) {
    ...
    if (!db_check_group_exists(db, group_id)) {
        ...
        if (res == 0) snprintf(response, RESPONSE_SIZE, "200 Request sent");
        else if (res == -2) snprintf(response, RESPONSE_SIZE, "409 Already joined/requested");
        else snprintf(response, RESPONSE_SIZE, "500 Internal Server Error");
    }
}
...
void handle_leave_group(MYSQL *db, int current_user_id, const char *arg1, char *response) {
    ...
    if (!db_check_group_exists(db, group_id)) {
        ...
        if (db_is_group_admin(db, current_user_id, group_id)) {
            ...
            snprintf(response, RESPONSE_SIZE, "200 Left group");
        }
    }
}
...
void handle_approve_req(MYSQL *db, int current_user_id, const char *arg1, const char *arg2, char *response) {
    ...
    if (mysql_query(db, query)) {
        snprintf(response, RESPONSE_SIZE, "500 Database error");
        return;
    }
    ...
    if (!row) {
        snprintf(response, RESPONSE_SIZE, "404 Request not found");
    ...
    if (type == 'A') {
        ...
        if (result == 0) {
            snprintf(response, RESPONSE_SIZE, "200 Request accepted, user added to group");
        } else {
            snprintf(response, RESPONSE_SIZE, "200 Request accepted but failed to add user to group");
        }
    } else if (type == 'R') {
        db_update_request_status(db, req_id, "rejected");
        snprintf(response, RESPONSE_SIZE, "200 Request rejected");
    } else {
        snprintf(response, RESPONSE_SIZE, "400 Bad Request");
    }
}
...
void handle_kick_member(MYSQL *db, int current_user_id, const char *arg1, const char *arg2, char *response) {
    ...
    if (!db_is_group_admin(db, current_user_id, group_id)) {
        ...
        if (target_id == -1) {
            snprintf(response, RESPONSE_SIZE, "404 User not found");
        } else {
            db_remove_group_member(db, target_id, group_id);
            snprintf(response, RESPONSE_SIZE, "200 User kicked");
        }
    }
}
...
void handle_invite_user(MYSQL *db, int current_user_id, const char *arg1, const char *arg2, char *response) {
    ...
    if (!db_check_group_exists(db, group_id)) {
        ...
        if (target_id == -1) {
            ...
            if (res == 0) snprintf(response, RESPONSE_SIZE, "200 Invitation sent");
            else if (res == -2) snprintf(response, RESPONSE_SIZE, "409 Already member/invited");
            else snprintf(response, RESPONSE_SIZE, "500 Error");
        }
    }
}
